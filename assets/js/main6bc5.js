...
function academy_datepicker(){
    ...
    //var allowDays = jQuery('input[name=allow_date_list]').val().split('|');

         // ['2017-9-22','2017-9-26','2017-9-30'];
...
let hasArticleDirectory = () => {
    let url = window.location.href;
    let urlStringArray = url.split('/');
    let urlString = urlStringArray[urlStringArray.length - 2];
    if(($('a[href$="'+ urlString +'/"]').length > 0) && !($('a[href$="../'+ urlString +'/"]').hasClass('current-blog'))){
        $('a[href$="'+ urlString +'/"]').addClass('current-blog');
    }
}
hasArticleDirectory();
